...
const LoanEmiScreen = ({ navigation }: any) => {
  ...
  const data = [
    { id: '1', paymentAmount: '$478399', date: '10th Jul. Loan', icon: 'checkmark-circle', status: 'Payment Successful' },
  ...
  const renderItem = ({ item }: any) => (
    ...
    >
      <View style={styles.cardContent}>
        <Ionicons name={item.icon} size={30} color={isDarkMode ? Colors.green : Colors.green} /> 
        <View style={styles.textContent}>
          <Text style={[styles.boldText, { color: isDarkMode ? Colors.white : Colors.black }]}>
            {item.status}
          </Text>
          <Text style={{ color: isDarkMode ? Colors.white : '#FFA500' }}>
            {item.paymentAmount}
          </Text>
        </View>
        <Ionicons name="chevron-forward" size={20} color={isDarkMode ? Colors.white : Colors.black} />
      </View>
